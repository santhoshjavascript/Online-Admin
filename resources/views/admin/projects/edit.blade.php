        ...
        <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description"
                          ...
                          rows="5" placeholder="Enter project description" required>{{ old('description', $project->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600 flex items-center"><i class='bx bx-error mr-1'></i>{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="form-group" id="new-category-group" style="display: {{ old('category_id', $project->category_id) ? 'none' : 'block' }};">
            ...
            <div class="form-group">
                ...
                @if ($project->abstract_url)
                    <p class="mt-2 text-sm text-gray-600">Current file: {{ basename($project->abstract_url) }} <a href="{{ Storage::url($project->abstract_url) }}" target="_blank" class="text-blue-600 hover:underline">View</a></p>
                @endif
                @error('abstract_url')
                    <p class="mt-2 text-sm text-red-600 flex items-center"><i class='bx bx-error mr-1'></i>{{ $message }}</p>
                @enderror
            ...
            <div class="form-group">
                ...
                @if ($project->thumbnail)
                    <p class="mt-2 text-sm text-gray-600">Current file: {{ basename($project->thumbnail) }} <a href="{{ Storage::url($project->thumbnail) }}" target="_blank" class="text-blue-600 hover:underline">View</a></p>
                @endif
                @error('thumbnail')
                    <p class="mt-2 text-sm text-red-600 flex items-center"><i class='bx bx-error mr-1'></i>{{ $message }}</p>
                @enderror
